...
export default function MissingProfessorButton({
  page = "professors",
  searchQuery,
  classQuery,
  show = true,
  ...
  const [classInput, setClassInput] = useState(classQuery || "")
  ...
  async function submit() {
    ...
    try {
      ...
      setStatus("sent")
      posthog.capture("missing_professor_submitted", {
        page,
        has_class: !!classInput.trim(),
    ...
    <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/60 p-4">
      <div className="w-full max-w-lg rounded-2xl bg-zinc-950 p-5 ring-1 ring-white/10">
        <div className="flex items-center justify-between">
          <h3 className="text-lg font-semibold">Report a missing professor</h3>
          <button
            onClick={() => {
              setOpen(false)
              setStatus("idle")
        ...
        <div className="mt-4 space-y-3">
          <div>
            <label className="text-sm text-white/70">Professor name</label>
            <input
              value={professorName}
              onChange={(e) => setProfessorName(e.target.value)}
              placeholder="ex: James Kosmach"
          ...
          <div>
            <label className="text-sm text-white/70">Class (optional)</label>
            <input
              value={classInput}
              onChange={(e) => setClassInput(e.target.value)}
              placeholder="ex: CS 301"
              className="mt-1 w-full rounded-xl bg-zinc-900 px-3 py-2 ring-1 ring-white/10"
            />
          </div>
          ...
          <div>
            ...
            <textarea
              ...
              placeholder="Anything that helps, nickname, spelling, term, section, etc"
              className="mt-1 w-full rounded-xl bg-zinc-900 px-3 py-2 ring-1 ring-white/10"
              rows={3}
            />
          </div>
          ...
          <div className="flex items-center gap-2 pt-2">
            <button
              onClick={submit}
              disabled={status === "sending" || professorName.trim().length < 2}
