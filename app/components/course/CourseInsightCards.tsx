...
function card(label: string, value: string, helper?: string) {
  return (
    <div className="rounded-3xl border border-zinc-200 bg-white p-5 shadow-sm dark:border-white/10 dark:bg-zinc-900/40">
      <div className="text-[11px] font-semibold uppercase tracking-wide text-zinc-500 dark:text-zinc-400">
        {label}
      </div>
      <div className="mt-2 text-2xl font-semibold tracking-[-0.01em] text-zinc-900 dark:text-zinc-100">
        {value}
      </div>
      {helper ? (
...
export default function CourseInsightCards({
  ...
  return (
    <section>
      <h2 className="mb-4 text-2xl font-semibold tracking-[-0.01em] text-zinc-900 dark:text-zinc-100">
        Quick Insights
      </h2>
